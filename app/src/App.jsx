...
function App() {
  ...
  const handleSubmit=async()=>{
    ...
    try{
      const req = await axios.post('https://compilador-1.onrender.com/api/code',code);
      await setOutput(req.data.output);
    }catch(error){
      console.error("Error de codigo: ", error);
      setOutput("Error al procesar solicitud."); //Mostrar mensaje de error al usuario
    }finally{
      setLoading(false);
    }
  }
    ...
    <div className="min-h-screen bg-[#1e1e1e] text-gray-200 p-6 font-mono">
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        <textarea
          ...
          rouded-md p-3 focus:outline-none focus:border-[#007acc]
          placerholder-gray-500 resize-none shadow-inner" placeholder="Escribe algo aqui..."
          ></textarea>
          ...
          disabled={loading}
          className={`px-4 py-2 rounded-r-md font-semibold transition
            ${loading
